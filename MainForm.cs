...
namespace STM32_FFT_Spectral_Analysis
{
	...
	public partial class MainForm : Form
	{
		...
		{
			stlink = new SerialPort(comboBox1.SelectedItem.ToString(),	115200, Parity.None,8,StopBits.One);
			stlink.ReadTimeout = 2000;
            stlink.Open();
            stlink.DataReceived += new SerialDataReceivedEventHandler(port_DataReceived);
		}
		
		private void port_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            int intBuffer;
            intBuffer = stlink.BytesToRead;
            byte[] byteBuffer = new byte[intBuffer];
            stlink.Read(byteBuffer, 0, intBuffer);
